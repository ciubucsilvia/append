...
class FeatureController extends AdminController
{
    ...
    public function index()
    {
        ...
        $items = $this->feature_repository->getAllWithPaginate();
    ...
    public function store(StoreFeatureRequest $request)
    {
        ...
        $client = $this->feature_repository->create($data);
        
        // save $request->file('image')
        $this->feature_repository->saveImage($client, $request, $this->folder);
        // save $data['active']
        $this->feature_repository->setActive($client, $data);
    ...
    public function update(UpdateFeatureRequest $request, Feature $feature)
    {
        ...
        // save $request->file('image')
        $this->feature_repository->saveImage($feature, $request, $this->folder);
        // save $data['active']
        $this->feature_repository->setActive($feature, $data);

        if($feature) {
            return redirect()
            ->route('admin.clients.index')
            ->with(['success' => "Client: $feature->name updated!"]);
        } else {
            return back()
                ->withErrors(['msg' => 'Save error!'])
                ->withInput();
    ...
    public function destroy(Feature $feature)
    {
        ...
        $this->feature_repository->deleteImage($feature->image);
        $feature->delete();
